# Knowledge Graph Query Reference (Obsidian)

Quick reference for querying the KG builder's SQLite output from the Obsidian
writing workflow. Use these queries to audit links, find gaps, and traverse the
knowledge graph while writing notes.

For the KG builder tool itself (Dockerfile, config, how to run), see
`tools/kg-builder/`.
